...
// === Lógica de Búsqueda en el Catálogo (SOLUCIÓN FINAL) ===

function setupCatalogSearch() {
    const searchInput = document.getElementById('catalog-search');
    const catalogContainer = document.getElementById('catalog-items-container');

    if (!searchInput || !catalogContainer) return; 

    // **CORRECCIÓN CLAVE:** Seleccionar todos los DIVs de producto que son hijos directos de un grid,
    // y que representan un ítem individual (son los que tienen la clase 'shadow-lg' en tu HTML)
    const productItems = catalogContainer.querySelectorAll('.grid > div');
    
    // Asegurar que todos los productos sean visibles inicialmente
    productItems.forEach(item => {
        // En lugar de 'block', usamos la propiedad que Tailwind usa para hacerlos visibles: flex/block, 
        // pero 'block' es seguro para el filtrado.
        item.style.display = 'block'; 
    });


    // Función de filtrado que se ejecuta con cada tecla pulsada
    searchInput.addEventListener('keyup', () => {
        ...
        productItems.forEach(item => {
            // 1. Obtener el texto completo del producto
            const itemText = item.textContent.toLowerCase();
            
            // 2. Determinar si el producto debe ser visible
            const isVisible = itemText.includes(searchText);

            // 3. Ocultar o mostrar el producto
            if (isVisible) {
                // Aquí usamos 'block' (o el valor que necesitas)
                item.style.display = 'block';
        ...
        // Lógica para ocultar/mostrar SECCIONES completas
        catalogContainer.querySelectorAll('div[id$="-section"]').forEach(section => {
            
            // Contar cuántos productos son visibles DENTRO de esta sección específica
            const visibleProductsInSection = section.querySelectorAll('div[style="display: block;"]').length;
            
            // Si hay productos visibles o si no se ha escrito nada en la búsqueda, muestra la sección.
            if (visibleProductsInSection > 0 || searchText === '') {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    setupCatalogSearch(); // Llama a la función de búsqueda
});
...
document.getElementById('contact-form-final').addEventListener('submit', function(event) {
    ...
    setTimeout(() => {
        ...
        // document.getElementById('contact-form-final').reset(); // CORREGIDO: Línea de reset eliminada
    }, 5000);
});
...
document.getElementById('quotation-form').addEventListener('submit', async function(event) {
    ...
    try {
        ...
        // form.reset(); // <<--- CORREGIDO: ¡ESTA LÍNEA SE HA ELIMINADO!
